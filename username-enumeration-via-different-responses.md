...
The lab gives you two wordlists: one for candidate usernames and one for candidate passwords.
...
In Intruder, selected the **Sniper** attack type. Cleared all default payload positions and manually marked only the `username` parameter as the payload position.
...
After the attack completed, sorted the results by **response length**. Most requests came back with the same response length and returned **"Invalid credentials"**. One username (`user`) produced a different response length and returned **"Incorrect password"**, indicating that the username existed and only the password was incorrect.
...
Fixed the valid username in the request and moved the payload position to the `password` parameter. Loaded the candidate passwords wordlist and launched another Sniper attack.
...
Used the discovered credentials to log in through the browser and successfully accessed the user account page.
